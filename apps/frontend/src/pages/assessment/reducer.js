  ...
  switch (action.type) {
    ...
    case "select-questionnaire":
      return {
        ...state,
        selectedQuestionnaires: (() => {
          const selected = state.selectedQuestionnaires.includes(action.payload);
          if(selected){
            return state.selectedQuestionnaires.filter( item => item !== action.payload )
          }
    ...
    case "delete-questionnaires":
      return {
        ...state,
        questionnaires: state.questionnaires.filter(item => !state.selectedQuestionnaires.includes(item._id)),
        selectedQuestionnaires: []
      }
    ...
    case "select-question":
      return {
        ...
        selectedQuestions: (() => {
          ...
          if(selected){
            return state.selectedQuestions.filter( item => item !== action.payload )
          }
        ...
        questionnaires: (() => {
          ...
          return questionnaires.map(questionnaire => {
            if(questionnaire._id === action.payload){
              const newQuestions = questionnaire.questionnaire.filter(question => 
