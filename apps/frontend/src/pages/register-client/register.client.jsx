...
import { Paper, Box, Typography, Grid, useMediaQuery, Container, Button, Divider } from "@mui/material";
...
export const RegisterCompanyDepartment = () => {
  ...
  const [companyID, setCompanyID] = useState(location.state ? location.state.companyID : null);
  ...
  useEffect(() => {
    ...
    dispatch({
      ...
      payload: {
        ...
        _id: clientReq.response.company.admin ? clientReq.response.company.admin : ""
      }
    });
  ...
  const handleSubmit = async () => {
    ...
    setTimeout(() => {
      executeWithData({
        ...
        id: state.company.id,
        admin: state.consultant.sleepScienceConsultant._id,
        id: companyID
      });
    }, 1000);
  };
  ...
  return (
    <Shell heading="Register Company Department">
      ...
      >
        ...
        >
          <Typography
            sx={{
              // width: "10%",
              fontSize: "14px"
            }}>Onboard a department</Typography>
          <Divider
            sx={{
              width: "90%",
              borderWidth: "1px",
        ...
        <InputContainer executeDispatch={executeDispatch} state={state} dispatch={dispatch}/>
        <Container
          sx={{
            display: "flex",
            justifyContent: "end",
