...
export const CustomTableSingleRow = ({
    ...
    useEffect(() => {
        ...
        if(error) {
            ...
            const message = count === 0 ? "There are no assessments completed by employees within this department.":"There has been an error trying to download the PDF.";
    ...
    return (
        <TableRow
            key={assessment._id}
            onClick={() => push(`/departments/${assessment._id}`)}
            sx={{
                backgroundColor: `${
                    selected.includes(assessment._id) ? "primary.main" : "white"
                }`,
                color: `${
                    selected.includes(assessment._id) ? "white" : "black"
                }`,
                marginBottom: "7%",
                cursor: "pointer",
            }}
        >
            ...
            >
                ...
                >
                    {selected.includes(assessment._id) ? (
                        <SelectedBlackIcon />
                    ) : (
                        <UnselectedIcon />
                    )}
            ...
            >
                <Typography sx={{ ...styles.tableRowText }}>
                    {assessment.lastAssessmentDate !== "none" ? format(
                        new Date(assessment.lastAssessmentDate),
                        "dd/MM/yyyy"
                            ...
                            <IconButton
                                onClick={(event) => {
                                    ...
                                    const request = createRequest({
                                        url: `/departments/group-report/${assessment._id}`,
                                        method: "get",
            ...
            >
                <IconButton
                    onClick={(event) => {
                        event.stopPropagation();
                        setOpenMessage(true);
                        setSelected((prev) => [...prev, assessment._id]);
                    }}
                >
