...
export const CustomTableEmployeeRow = ({
  ...
  const getPDF = useAxios({
    url: `/employees/report/${employee.id}`,
    method: 'get',
  });
  ...
  return (
    <TableRow
      ...
      onClick={() => {
        ...
        push(`/employee/${employee.id}`, [{ departmentId }]);
      }}
      sx={{
        backgroundColor: `${
          selected.includes(employee.id) ? 'primary.main' : 'white'
        }`,
        color: `${selected.includes(employee.id) ? 'white' : 'black'}`,
        marginBottom: '7%',
        cursor: 'pointer',
      }}
    >
          ...
          <IconButton
            onClick={(event) => {
              ...
              const request = getPDF.createRequest({
                url: `/employees/individual-report/${employee.id}`,
                method: 'get',
